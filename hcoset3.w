...
void doouter(int r) {
   ...
   for (int i=0; i<STRIDE; i++) {
      ...
      for (int mv=0; mv<NMOVES_EXT; mv++) {
         if (!kocsymm::in_Kociemba_group(mv))
            continue ;
         e->from[dp] = BITINDEX(bitp2, neighbors[dp]) ;
         if (mv == moveseq16[i])
            tcperm = neighbors[dp] ;
         dp++ ;
      }
...
void showunset(int cperm) {
   int *pbits = (int *)BITINDEX(bitp1, cperm) ;
   permcube pc ;
   pc.c8_4 = cperm / (FACT4 * FACT4) ;
   pc.ctp = cperm / FACT4 % FACT4 ;
   pc.cbp = cperm % FACT4 ;
...
void finish_prepass_work(int cperm) {
...
#endif
   {
      ...
      if (need_count_bits > 1)
         thisblock = countbits2(cperm, (int *)BITINDEX(bitp1, cperm), this_ulev) ;
      else
#endif
      thisblock = countbits((unsigned int *)BITINDEX(bitp1,cperm)) ;
   }
 ...
 */
   ...
   c.p = BITINDEX(bitp1, cindex) + (eindex >> 3) ;
   prefetch(c.p) ;
#ifdef LEVELCOUNTS
 ...
 */
   ...
   while (movemask) {
      ...
      kc2 = kc ;
      kc2.move(mv) ;
      int nd = phase1prune2::lookup(kc2, togo, newmovemask, fi) ;
      if (nd == togo && fi.initstate >= 0) {
         int new_canon = cubepos::next_cs(canon, mv) ;
         int movemask3 = newmovemask & cubepos::cs_mask(new_canon) ;
         if (movemask3 != 0) {
            pc2 = pc ;
            ...
            int ok = 1 ;
            int ntogo = togo ;
            if (ntogo > 2 && (enc >> ((ntogo - 3) * 4)) != 0) {
               while (1) {
                  s = phase1prune2::extract_moves[s][enc & 15] ;
                  int mv = s & 31 ;
                  if (((cubepos::cs_mask(new_canon) >> mv) & 1) == 0) {
                     ok = 0 ;
                     break ;
                  }
                  pc2.move(mv) ;
                  ntogo-- ;
                  new_canon = cubepos::next_cs(new_canon, mv) ;
                  s >>= 5 ;
                  if (enc == 0)
                     break ;
                  enc >>= 4 ;
            ...
            if (ok) {
               if (ntogo == 1) {
                  int onemove = phase1prune2::forced_single[pc2.em] ;
                  pc2.move(onemove) ;
                  local_setonebit(pc2) ;
                  pc2.move(onemove) ;
                  pc2.move(onemove) ;
                  ...
                  phase1prune2::lookup(kc3, ntogo, movemask3) ;
                  search(kc3, pc2, ntogo,
                        movemask3 & cubepos::cs_mask(new_canon), new_canon) ;
               }
            }
         }
      } else if (nd <= togo && (
...
set of three moves to work with.  Note that if the starting depth
is four or less, we do not even bother to spread the work.
...
int get_search_work() {
   ...
   while (1) {
      r = search_work_seq++ ;
      if (r >= NMOVES*NMOVES*NMOVES) {
         r = -1 ;
         break ;
      }
      int mv1 = r / NMOVES / NMOVES ;
      int mv2 = r / NMOVES % NMOVES ;
      int mv3 = r % NMOVES ;
...
void dowork() {
   ...
   if (global_depth <= 4) {
      ...
      while (1) {
         int movetrip = get_search_work() ;
         if (movetrip < 0)
            break ;
         int mv1 = movetrip / NMOVES / NMOVES ;
         int mv2 = movetrip / NMOVES % NMOVES ;
         int mv3 = movetrip % NMOVES ;
         kocsymm kc(repkc) ;
         permcube pc(reppc) ;
         kc.move(mv1) ;
         pc.move(mv1) ;
         ...
         int s = cubepos::next_cs(CANONSEQSTART, mv1) ;
         s = cubepos::next_cs(s, mv2) ;
         s = cubepos::next_cs(s, mv3) ;
         int mask ;
         phase1prune2::lookup(kc, global_depth-3, mask) ;
         search(kc, pc, global_depth-3, mask & cubepos::cs_mask(s), s) ;
      }
   }
   local_bitflush() ;
   get_global_lock() ;
...
void search() {
   duration() ;
   for (int d=phase1prune2::lookup(repkc); d <= maxdepth; d++) {
      global_depth = d ;
      probes = 0 ;
      long long prevlev = uniq ;
#ifdef LEVELCOUNTS
...
void docoverelement(int seq, const kocsymm &kc) {
   ...
   if (d > maxdepth) // skip if too deep
      return ;
   moveseq moves = phase1prune2::solve(kc) ;
